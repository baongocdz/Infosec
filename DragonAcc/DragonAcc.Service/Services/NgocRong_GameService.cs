...
namespace DragonAcc.Service.Services
{
    public class NgocRong_GameService : BaseService, INgocRong_GameService
    {
        ...
        private async Task<List<string>> UploadFiles(int? accountId, List<IFormFile>? files)
        {
            ...
            var accountFolder = $"public/NgocRong_Game/{accountId}";
        ...
        {
            var gameAccount = await _dataContext.NgocRong_Games.FirstOrDefaultAsync(x => x.Id == model.Id);
                ...
                try
                {
                    if (gameAccount.Status == "Đang chờ duyệt")
                    {
                        ...
                        var notification = new Notification
                        {
                            ...
                            UserId = gameAccount.UserId,
                            Content = "Tài khoản ngọc rồng online của bạn đã được duyệt.",
                            IsRead = false,
                            CreatedDate = DateTime.Now,
                        };
        ...
        public async Task<ApiResult> BuyGameAccount(BuyAccountNgocRong_GameModel model)
        {
            ...
            try
            {
                ...
                var notification = new Notification
                {
                    ...
                    UserId = seller.Id,
                    Content = $"{buyer.FullName ?? buyer.UserName} đã mua tài khoản ngọc rồng online của bạn.",
                    IsRead = false,
                    CreatedDate = DateTime.Now,
                };
                _dataContext.Notifications.Add(notification);

...
{
    public class Valorant_GameService : BaseService, IValorant_GameService
    {
        private readonly IFtpDirectoryService _ftpDirectoryService;

        public Valorant_GameService(DataContext dataContext, IFtpDirectoryService ftpDirectoryService, IUserService userService)
        ...
        private async Task<List<string>> UploadFiles(int? accountId, List<IFormFile>? files)
        {
            ...
            var accountFolder = $"public/Valorant_Game/{accountId}";
        ...
        public async Task<ApiResult> AddForUser(AddValorant_GameModel model)
        {
            var gameAccount = await _dataContext.Valorant_Games.FirstOrDefaultAsync(x => x.AccountName == model.AccountName);
            ...
            if (gameAccount == null)
            {
                ...
                try
                {
                    var newGameAccount = new Valorant_Game
                    {
                        GameName = "Valorant",
                    ...
                    _dataContext.Valorant_Games.Add(newGameAccount);
        ...
        {
            var gameAccount = await _dataContext.Valorant_Games.FirstOrDefaultAsync(x => x.Id == id);
            if (gameAccount != null)
            {
                using var tran = await _dataContext.Database.BeginTransactionAsync();
                try
                {
                    _dataContext.Valorant_Games.Remove(gameAccount);
                    await _dataContext.SaveChangesAsync();
                    await tran.CommitAsync();
                    return new ApiResult();
                }
        ...
        {
            var result = await _dataContext.Valorant_Games.ToListAsync();
            return new ApiResult(result);
        }

        public async Task<ApiResult> GetById(int id)
        {
            var result = await _dataContext.Valorant_Games.FirstOrDefaultAsync(x => x.Id == id);
            return new ApiResult(result);
        }

        public async Task<ApiResult> UpdateForUser(UpdateValorant_GameModel model)
        {
            var gameAccount = await _dataContext.Valorant_Games.FirstOrDefaultAsync(x => x.Id == model.Id);
        ...
        {
            var game = await _dataContext.Valorant_Games.FirstOrDefaultAsync(x => x.Id == id);
            if (game == null)
            {
        ...
        {
            var result = await _dataContext.Valorant_Games
               .Where(x => x.UserId == userId)
               .ToListAsync();
            return new ApiResult(result);
        }

        public async Task<ApiResult> BuyGameAccount(BuyAccountValorant_GameModel model)
        {
            if (model.Id == null)
            {
